...
#+begin_src emacs-lisp
  ...
  (unbind-key "M-SPC" global-map)
  (define-key global-map (kbd "M-SPC") (make-sparse-keymap))
...
** copilot
Uses GitHub's copilot as completion system. Requires paid account,
because of this define a custom variable that controls wether to
enable it or not
#+begin_src emacs-lisp
  (defcustom hesam-copilot-enabled nil
    "Enable copilot-mode. Requires restart to take effect."
    :type 'boolean
    :group 'hesam)

  (cond ((and hesam-copilot-enabled
           (file-directory-p "/home/hesam/repos/copilot.el"))
        (use-package editorconfig)
        (use-package copilot
          :ensure-system-package nodejs
          :load-path "/home/hesam/repos/copilot.el"
          :hook
          ((org-mode prog-mode text-mode) . copilot-mode)
          ((org-mode prog-mode text-mode) . (lambda ()
                               (setq-local copilot--indent-warning-printed-p t)))
          :bind (:map copilot-completion-map
                      ("C-c n" . copilot-accept-completion)))))
#+end_src
...
#+begin_src emacs-lisp :tangle (if (eq system-type 'darwin) (concat user-emacs-directory "init.el") "no")
  (global-set-key (kbd "<f16>") (kbd "M-a"))
  (global-set-key (kbd "<f15>") (kbd "M-c"))
  (global-set-key (kbd "<f13>") (kbd "M-h"))
  (global-set-key (kbd "<f14>") (kbd "M-n"))
#+end_src
...
*** gptel
#+begin_src emacs-lisp
  (defcustom hesam-openai-key nil
    "OpenAI API key"
    :type 'string
    :group 'hesam)
  ...
  (if hesam-openai-key
      (use-package gptel
        :init
        (setq gptel-api-key hesam-openai-key
              gptel-default-mode 'org-mode)
        :bind ("M-SPC c" . gptel-menu)
        :config
        (add-to-list 'display-buffer-alist
                     '("\\*ChatGPT.*\\*"
...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "M-SPC v" 'vterm-other-window)
#+end_src
...
*** Ligatures
Ligatures are a way to display multiple characters as a single glyph. Here we use the package `ligature` to enable ligatures in Emacs. We use the font Iosevka Term Nerd Font which has ligatures built-in. We also set the ligatures to be enabled in programming modes.
#+begin_src emacs-lisp
