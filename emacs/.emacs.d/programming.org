...
*** style
Set the default style for C programming.
...
#+BEGIN_SRC emacs-lisp
  (use-package rustic
    ...
    (setq rustic-ansi-faces (vector
                             ...
                             dracula-cyan
                             dracula-white))
    :hook (rustic-mode . rustic-mode-auto-save-hook)) ; function defined below
  #+END_SRC

`rustic` can use `rustfmt` after we save buffers which we hooked above.
#+BEGIN_SRC emacs-lisp
