...
** window management
#+BEGIN_SRC emacs-lisp
  (general-create-definer leader-windmove
    :prefix "C-c w")
  ...
  (general-create-definer leader-tab
    :prefix "C-c t")

  (leader-tab
    "N" 'tab-move
    "P" 'hesam/tab-move-left
    ...
    "w" 'tab-close
    "o" 'tab-recent
    "0" 'hesam/tab-select-0
    "1" 'hesam/tab-select-1
    "2" 'hesam/tab-select-2
    "3" 'hesam/tab-select-3
    "4" 'hesam/tab-select-4
    "5" 'hesam/tab-select-5
    "6" 'hesam/tab-select-6
    "7" 'hesam/tab-select-7
    "8" 'hesam/tab-select-8
    "9" 'hesam/tab-select-9)

  (defvar tab-cycle
    (let ((map (make-sparse-keymap)))
      (define-key map (kbd "p") #'tab-previous)
      (define-key map (kbd "n") #'tab-next)
      (define-key map (kbd "w") #'tab-close)
      (define-key map (kbd "P") #'hesam/tab-move-left)
      (define-key map (kbd "N") #'tab-move)
      (define-key map (kbd "o") #'tab-recent)
      (dotimes (n 10)
        (define-key map (kbd (format "%d" n))
          (intern (format "hesam/tab-select-%d" n))))
      map))

  (defun hesam/tab-move-left (&optional arg)
    "Move the current tab to the left.
  With a prefix ARG, move the tab ARG positions to the left."
    (interactive "p")
    (tab-move (- (or arg 1))))

  (dolist (cmd '(tab-previous tab-next tab-close tab-move hesam/tab-move-left tab-recent))
    (put cmd 'repeat-map 'tab-cycle))

  ;; Defines the following for n in [1,8]
  ;; (defun hesam/tab-select-n ()
  ;;   (interactive)
  ;;   (tab-select n))
  (dotimes (n 8)
    (let ((fn (intern (format "hesam/tab-select-%d" (1+ n)))))
      (fset fn
            (lambda ()
              (interactive)
              (tab-select (1+ n))))))
  ...
  ;; Defines hesam/tab-select-n for in [1,8]
  (dotimes (n 8)
    (let* ((fn (intern (format "hesam/tab-select-%d" (1+ n))))
           (tab-number (1+ n)))
      (eval `(defun ,fn ()
  ...
  (defun hesam/tab-select-0 ()
    (interactive)
    (tab-recent))

  (defun hesam/tab-select-9 ()
    (interactive)
    (tab-last))

  ;; Does the following for n in [1,8]
  ;; (defvar tab-select-repeat-map
  ;;   (let ((map (make-sparse-keymap)))
  ;;     (define-key map "1" 'hesam/tab-select-1)
  ;;      ...
  (defvar tab-select-repeat-map
    (let ((map (make-sparse-keymap)))
      (dotimes (n 10)
  ...
  ;;Attach the repeat map to each command for n in [1,8]
  ;; (put 'hesam/tab-select-1 'repeat-map 'tab-select-repeat-map)
  (dotimes (n 9)
    (let ((fn (intern (format "hesam/tab-select-%d" n))))
      (put fn 'repeat-map 'tab-select-repeat-map)
      (put fn 'repeat-map 'tab-cycle)))
#+END_SRC
** global
#+BEGIN_SRC emacs-lisp
