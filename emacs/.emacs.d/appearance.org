...
* Borders and margins
#+BEGIN_SRC emacs-lisp
  (defun set-internal-border-width (frame)
    "Set the internal border width for the given FRAME."
    (with-selected-frame frame
      (set-frame-parameter nil 'internal-border-width 10)))

  (add-hook 'after-make-frame-functions 'set-internal-border-width)

  (defun hesam/set-window-margins ()
    "Set the window margins for the current buffer."
    (setq left-margin-width 1)
    (setq right-margin-width 1)
    (set-window-buffer nil (current-buffer)))

  (add-hook 'window-configuration-change-hook 'hesam/set-window-margins)
#+END_SRC
