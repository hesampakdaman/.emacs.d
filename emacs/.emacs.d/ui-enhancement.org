...
#+BEGIN_SRC emacs-lisp
  (add-hook
   'after-load-theme-hook
   (lambda (&optional args) (set-face-attribute 'fringe nil :background nil)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defun hesam/set-tab-face-attributes ()
    ;; We prefer it to be slighly smaller than surrounding text. We'll
    ;; also add a line across the Emacs frame to separate the tab bar
    ;; from the buffer. Inheriting nil to make sure it does not get
    ;; values from `'default` which would alter size of tab bar font
    ;; when zooming in/out the buffer.
  ...
  (add-hook
   'after-load-theme-hook
   (lambda (theme) (hesam/set-tab-face-attributes)))
#+END_SRC
