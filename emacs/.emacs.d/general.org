...
numbered backups. If we wanted to keep only the 100th latest revisions
are kept then add the following
```
...
below.
#+BEGIN_SRC emacs-lisp
  (setq backup-directory-alist `(("." . ,(concat (eval user-emacs-directory) "backups/")))
        backup-by-copying t
        version-control t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (setq auto-save-file-name-transforms
        `((".*" ,(concat (eval user-emacs-directory) "autosaves/") t)))
#+END_SRC
