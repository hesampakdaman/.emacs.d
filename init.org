...
#+begin_src emacs-lisp
  ...
  (defun hesam--get-system-state ()
    ...
    (let ((system-state (shell-command-to-string "darkman get")))
      (cond
       ((string-match-p "light" system-state) "light")
       ((string-match-p "dark" system-state) "dark")
       (t hesam--darkmode-state))))
  ...
  (defun hesam--toggle-matugen ()
    (interactive)
    (when-let* ((new-state (if (string= (hesam--get-system-state) "dark") "light" "dark")))
  ...
  (use-package solar
    :ensure nil
    :init
    (setq calendar-location-name "Stockholm, Sweden"))

  (defun hesam--is-daylight-p ()
    "Return non-nil if current time is between sunrise and sunset in Stockholm."
    (require 'solar)
    (let* ((now (decode-time))
           (current-float (+ (nth 2 now) (/ (nth 1 now) 60.0)))
           (sun-data (solar-sunrise-sunset (calendar-current-date)))
           (sunrise-list (car (car sun-data)))
           (sunset-list (car (nth 1 sun-data)))
           (sunrise-float (+ (car sunrise-list) (/ (nth 1 sunrise-list) 60.0)))
           (sunset-float (+ (car sunset-list) (/ (nth 1 sunset-list) 60.0))))
      (and (>= current-float sunrise-float) (< current-float sunset-float))))

  (defun hesam--apply-matugen-by-solar ()
    "Updates matugen based on Stockholm's sunrise/sunset. Triggered by hypridle."
    (interactive)
...
#+begin_src emacs-lisp
  ...
  (defun hesam/pick-ef-theme-by-hsl (primary-hsl surface-hsl)
    ...
    (require 'ef-themes)
    (let* ((primary (hesam/parse-hsl primary-hsl))
           (surface (hesam/parse-hsl surface-hsl))
           (hue (nth 0 primary))
           (sat (nth 1 primary))
