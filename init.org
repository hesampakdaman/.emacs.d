...
#+begin_src emacs-lisp
  ...
  (cl-defmacro system-pkg (pkg &key exe)
    "Queue PKG or EXE for installation if it doesn't exists in PATH.

  EXE may be a symbol or string naming the executable to check with
  `executable-find`.

  PKG may be a symbol or string naming the package to install, if different."
    (declare (indent 1))
    `(when system-pkg-mode
       (unless (executable-find (system-pkg--name ',(or exe pkg)))
         (cl-pushnew
          (system-pkg--name ',pkg)
          system-pkg--queue
          :test #'string=))))
